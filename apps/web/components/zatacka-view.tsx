...
export function ZatackaView({ snapshot, onInput }: { snapshot: ClientRoomSnapshot; onInput: (input: ZatackaControlInput) => void }) {
  ...
  useEffect(() => {
    ...
    for (const rider of game.riders) {
      ctx.fillStyle = rider.alive ? rider.color : '#64748b';
      ctx.beginPath();
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap gap-3 rounded-[22px] border border-white/10 bg-white/[0.04] p-4 text-sm text-white/70">
        <div>Round: <span className="font-semibold text-white">{game.round}</span></div>
        <div>State: <span className="font-semibold text-white">{game.phase}</span></div>
        <div>Alive: <span className="font-semibold text-white">{game.riders.filter((rider) => rider.alive).length}</span></div>
      </div>
      <canvas ref={canvasRef} width={960} height={600} className="h-auto w-full rounded-[28px] border border-white/10 bg-slate-950 shadow-[0_30px_80px_rgba(0,0,0,0.35)]" />
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {game.riders.map((rider) => (
          <div key={rider.id} className="surface-tile p-3 text-sm">
            <div className="flex items-center gap-2 font-semibold text-white">
              <span className="inline-block h-3 w-3 rounded-full" style={{ backgroundColor: rider.color }} />
              {rider.name}
            </div>
            <div className="mt-2 text-white/50">{rider.alive ? 'Alive' : 'Crashed'} · {rider.controlPreset.toUpperCase()}</div>
          </div>
        ))}
      </div>
    </div>
